...
internal sealed class SubtitleExtraction(IConfiguration configuration, IHostEnvironment environment, FfmpegLocator? locator = null) : IDisposable
{
    private readonly FfmpegLocator _locator = locator ?? new FfmpegLocator(new MediaOptions
    {
    ...
    {
        try { return binary == "ffmpeg" ? _locator.ResolveFfmpeg() : _locator.ResolveFfprobe(); }
        catch (FfmpegBinaryMissingException ex) { throw new IOException(ex.Message, ex); }
    }
