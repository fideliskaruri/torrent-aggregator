...
public static partial class QueryVariants
{
    ...
    public static string Canonicalize(string raw) => Ws().Replace((raw ?? "").Normalize(NormalizationForm.FormKC), " ").Trim().ToLowerInvariant();

    public static string Display(string raw) => Ws().Replace((raw ?? "").Normalize(NormalizationForm.FormKC), " ").Trim();
...
public static partial class Relevance
{
    ...
    {
        var d = (value ?? "").Normalize(NormalizationForm.FormKD);
